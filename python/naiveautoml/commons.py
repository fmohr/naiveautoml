...
import time
import pynisher
from .evaluators import LccvValidator, KFold, Mccv, EarlyDiscardingValidator
    ...
    def get_evaluation_fun(self, evaluation_fun):
        ...
        if evaluation_fun is None:
            if is_small_dataset:
                ...
                return Mccv(task.inferred_task_type, n_splits=5, random_state=self.random_state)
            elif is_medium_dataset:
                self.logger.info("This is a medium dataset, choosing mccv-3 for evaluation")
                return Mccv(task.inferred_task_type, n_splits=3, random_state=self.random_state)
            elif is_large_dataset:
                self.logger.info("This is a large dataset, choosing mccv-1 for evaluation")
                return Mccv(task.inferred_task_type, n_splits=1, random_state=self.random_state)
            else:
                raise ValueError(
                    "Invalid case for dataset size!! This should never happen. Please report this as a bug.")

        elif evaluation_fun == "lccv-80":
            return LccvValidator(task.inferred_task_type, train_size=0.8, random_state=self.random_state)
        elif evaluation_fun == "lccv-90":
            return LccvValidator(task.inferred_task_type, train_size=0.9, random_state=self.random_state)
        elif evaluation_fun == "kfold_5":
            return KFold(task.inferred_task_type, n_splits=5, random_state=self.random_state)
        elif evaluation_fun == "kfold_3":
            return KFold(task.inferred_task_type, n_splits=3, random_state=self.random_state)
            ...
            return EarlyDiscardingValidator(task.inferred_task_type, stopper="lce", random_state=self.random_state)
        elif evaluation_fun == "pfn":
            return EarlyDiscardingValidator(task.inferred_task_type, stopper="pfn", random_state=self.random_state)
