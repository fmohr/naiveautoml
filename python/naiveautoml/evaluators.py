    ...
    def __call__(self, pl, X, y, scorings, errors="raise"):
        ...
        try:
            ...
            if self.instance.task_type == "classification":
                splitter = sklearn.model_selection.StratifiedKFold(n_splits=self.n_splits, random_state=None, shuffle=True)
...
class Mccv:
    ...
    def __call__(self, pl, X, y, scorings, errors="raise"):
        ...
        try:
            ...
            if self.instance.task_type == "classification":
                splitter = sklearn.model_selection.StratifiedShuffleSplit(n_splits=self.n_splits, train_size=0.8, random_state=None)
            elif self.instance.task_type:
                splitter = sklearn.model_selection.ShuffleSplit(n_splits=self.n_splits, train_size=0.8, random_state=None)
            scores = {get_scoring_name(scoring): [] for scoring in scorings}
            for train_index, test_index in splitter.split(X, y):
