...
class geneticalgorithm():
    ...
    def __init__(self, function, dimension, variable_type='bool', \
       ...
       #############################################################
        ...
        self.param={'max_num_iteration': None,\
                                       'population_size':100,\
        ...
        for k in algorithm_parameters:
            self.param[k] = algorithm_parameters[k]
    ...
    def add_init_solution(self,x):
        self.init_pop.append(x)
    ...
    def run(self):
        ...
        if "timeout" in self.param:
            deadline = time.time() + self.param["timeout"]
        else:
            deadline = np.inf
        ...
        for i, p in enumerate(self.init_pop):
            obj = self.sim(p)
            var = list(p) + [obj]
            pop[i] = np.array(var)
            print("Considered init solution " + str(pop[i]))
            if self.best_function is None or obj < self.best_function:
                self.best_function = obj
                self.best_variable = var
        
        for p in range(len(self.init_pop),self.pop_s):
        ...
        #############################################################
        print("GA initialization complete!")
        ...
        while t<=self.iterate:
            ...
            print("Now starting iteration #" + str(t) + " of the GA. Remaining time: " + str(remaining_time) + ". Best score is " + str(self.best_function) + " obtained by individual " + str(self.best_variable))
